...
class ProgramGUI:
    ...
    reveal_punchline:tk.Button = None
    reveal_punchline_label:tk.StringVar = None
    ...
    def show_punchline(self):
        ...
        if joke != None:
            self.reveal_punchline_label.set("Loading...")
            # 3 second delay
            self.window.after(3000, self.show_punchline_delayed)

    def show_punchline_delayed(self):
        joke:Joke = self.data[self.currentJoke]
        if joke != None:
            self.reveal_punchline.destroy()
    ...
    def showJoke(self):
        ...
        if joke != None:
            ...
            self.reveal_punchline_label = tk.StringVar(value="Reveal Punchline")

            self.reveal_punchline = tk.Button(self.main_frame, textvariable=self.reveal_punchline_label, command=self.show_punchline)
            self.reveal_punchline.grid(row=1,column=1,columnspan=3)
            ...
            font = ("Arial", 14)
            self.users_rating = tk.StringVar(value="1")
            ...
            w = tk.OptionMenu(self.main_frame, self.users_rating, "1", "2", "3","4","5")
            w.grid(row=3,column=2,sticky="s")
            w.focus()
